...
import SiteManagerServices from "./SiteManagerServices";
...
function SMCommunication() {
  ...
  const [preview, setPreview] = useState(null); // mobile modal preview
  const [selected, setSelected] = useState(null); // desktop right preview
  ...
  const openMessage = async (msg) => {
    try {
      // fetch single message so server can auto-mark as read when recipient views
      const res = await axios.get(`${BASE_URL}/messages/${msg._id}`, { headers: token ? { Authorization: `Bearer ${token}` } : {} });
      const message = res.data.message || res.data;
      if (isMobile) setPreview(message);
      else setSelected(message);
      // refresh lists so read state updates
      await fetchMy();
      await fetchSent();
      // notify nav and other components
      try { window.dispatchEvent(new CustomEvent('messageRead', { detail: { messageId: message._id } })); } catch (e) { }
    } catch (err) {
      // fallback: show passed message and try to mark as read
      if (isMobile) setPreview(msg);
      else setSelected(msg);
      if (messagesTab === 'my' && !msg.isRead) {
        axios.patch(`${BASE_URL}/messages/${msg._id}/read`, {}, { headers: token ? { Authorization: `Bearer ${token}` } : {} }).catch(()=>{});
      }
    }
  };
  ...
  return (
    <div className="px-6 py-8">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
        <div>
          <h1 className="text-3xl md:text-2xl font-bold text-[#0B3954]">Site Communication</h1>
          <p className="text-gray-600 mt-2">Announcements and team messages for your sites.</p>
        </div>
        {activeTab === 'messages' && (
                        ...
                        <div className="flex items-center gap-2">
                          {messagesTab === 'sent' && (
                            <div className="text-xs mr-2">
                              <span className={`px-2 py-0.5 rounded ${msg.isRead ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>{msg.isRead ? 'Read' : 'Unread'}</span>
                              {msg.readAt && msg.isRead && <span className="ml-2 text-gray-400 text-xs">{new Date(msg.readAt).toLocaleString()}</span>}
                            </div>
                          )}
                          <button
                      ...
                      <div className="p-4 border rounded-lg bg-gray-50">
                        ...
                        {selected ? (
                          <div className="mt-3">
                            ...
                            <p className="text-xs text-gray-500">{selected.sender} • {selected.date ? new Date(selected.date).toLocaleDateString() : ''}</p>
      ...
      {activeTab === 'services' && <SiteManagerServices />}
...
export default SMCommunication;
