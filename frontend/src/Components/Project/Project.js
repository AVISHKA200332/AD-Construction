...
function Project({ initialProjects = [] }) {
  ...
  const handleEdit = (index) => {
    ...
    setNewProject({
      ...
      clientContact: projectToEdit.clientContact || { phone: "", email: "", bankAccount: "" },
      projectManager: projectToEdit.projectManager || { name: "", age: "", experience: "" },
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-3xl font-bold text-gray-800">AD Construction Project Management</h1>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          ...
          <div className="bg-white p-4 rounded-lg shadow-sm border">
            ...
            <p className="text-2xl font-bold text-blue-600">
              {projectStats.distribution?.find(d => d.status === 'In Progress')?.count || 0}
            </p>
          </div>
          <div className="bg-white p-4 rounded-lg shadow-sm border">
            <h3 className="text-sm font-medium text-gray-500">Completed</h3>
            <p className="text-2xl font-bold text-green-600">
              {projectStats.distribution?.find(d => d.status === 'Completed')?.count || 0}
            </p>
          </div>
        </div>
      )}
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody>
              {loading ? (
                ...
                projects.map((p, idx) => (
                  <tr key={p._id || idx} className="border-b last:border-0 hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <div>
                        ...
                        {p.clientContact?.phone && (
                          <div className="text-xs text-gray-500">{p.clientContact.phone}</div>
                        )}
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      ...
                      <div className="w-full bg-gray-200 rounded-full h-2 mb-1">
                        <div
                          className={`h-2 rounded-full ${
                            p.completion >= 100 ? 'bg-green-500' : 
                            p.completion >= 75 ? 'bg-blue-500' : 
                            p.completion >= 50 ? 'bg-yellow-500' : 'bg-red-500'
                          }`}
                    ...
                    <td className="px-4 py-3">
                      {p.projectManager?.name ? (
                        <div>
                          <div className="font-medium text-sm">{p.projectManager.name}</div>
                          {p.projectManager.experience && (
                            <div className="text-xs text-gray-500">{p.projectManager.experience} years exp</div>
                          )}
                        </div>
                      ) : (
